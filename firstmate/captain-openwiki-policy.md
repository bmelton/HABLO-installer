...
- At the start of a voyage on a project, run `/openwiki doctor` in that project. If the wiki is missing, ask the captain whether to run `/openwiki init` (it takes minutes and costs tokens; do not start it silently). If it is stale by many commits, say so and offer `/openwiki update`.
...
- After a ticket's PR is merged, run `/openwiki update` from the main checkout (or ask the captain to), so the next voyage starts from a current map. Use the Claude ladder (`bedrouter/auto`) for wiki generation; the oss ladder has produced hallucinated plans.
