...
0. **Fresh machine?** First the AWS profile (README, "Before you run it"): `aws configure sso --profile bedrouter`, then `./install.sh --install-pi --profile bedrouter`. The rest of this list assumes that has run.
...
7. **firstmate.** `cd ~/firstmate && pi --provider bedrouter --model auto`, ask for a status report; `/bearings` shows the crew. Crewmates inherit the captain's exact provider/model from `config/crew-dispatch.json` (check once with `/bedrouter status` in a crewmate's tmux window). Give every task a Jira key; the captain policy makes crewmates branch from and PR into `<JIRA-KEY>`, never main.
8. **Jira demo.** Map the real project key in `hablo.json`, populate `~/.hablo/jira/.env`, and run `hablo-jira doctor --key <JIRA-KEY>`. Add `agent-ready` to a ticket assigned to the token owner, run `hablo-jira-agent tick`, confirm the label becomes `agent-running`, then attach with `tmux attach -t hablo-<JIRA-KEY>`.
9. **Dream.** Run `hablo-dream doctor`, then `hablo-dream digest`. Inspect the redacted digest under `~/.hablo/dream/`; a model-backed `hablo-dream run` should create a validated report without writing any memory or policy file.
10. **Kill switch.** `/bedrouter stop` and `/bedrouter start` both work from inside Pi; `BEDROUTER_DEBUG=1` in `~/.bedrouter/.env` if you want the per-request trace in `server.log`.
