...
7. **firstmate.** `cd ~/firstmate && pi`, say `ahoy!`; `/bearings` shows the crew. If crewmates should be routed too, the installer must have been run with `--default-model` (check: `/bedrouter status` in a crewmate's tmux window shows `using auto`).
