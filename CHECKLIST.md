...
9. **Kill switch.** `/bedrouter stop` and `/bedrouter start` both work from inside Pi; `BEDROUTER_DEBUG=1` in `~/.bedrouter/.env` if you want the per-request trace in `server.log`.
